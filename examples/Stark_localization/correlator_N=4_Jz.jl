...
function get_Liouvillian(Jx,Jy,Jz,gamma)
  N=4
  L=2*N
 
  H=get_Liouvillian_spin_chain_Jz(Jx,Jy,gamma,Jz,N=N,mode="MPO")
...
function get_Liouvillian_DOS(i,H,psi_1,Jx,Jy,Jz,gamma)
  nEx=41
  nEy=41
  ...
  open(string("correlator_Jz_N=4_Sz7_Jx=",string(Jx),"_Jy=",string(Jy),"_Jz=",string(Jz),"_gamma=",string(gamma),"_index=",string(i),".OUT"),"w") do io
...
Jz=1
gamma=0.2
H, psi_1=get_Liouvillian(Jx,Jy,Jz,gamma)
i=parse(Int,ARGS[1])
for j=i:i+4
  get_Liouvillian_DOS(j,H,psi_1,Jx,Jy,Jz,gamma)
end
